...
try {
  ...
  console.log(
    pc.red(
      'Could not read git remote origin. Make sure this is a git repository with a remote named "origin".'
    )
  )
  console.log()
  process.exit(1)
...
const configuredMatch = snippet.match(
  /cdn\.jsdelivr\.net\/gh\/([\w.-]+\/[\w.-]+)@/
)
...
if (configuredPath === repoPath) {
  ...
  console.log(
    pc.green(
      `${pc.bold('✓')} Already configured for ${pc.bold(repoPath)} — nothing to do.`
    )
  )
  console.log()
  process.exit(0)
...
if (!configuredPath) {
  ...
  console.log(
    pc.red(
      'Could not detect the currently-configured repo path in webflow-snippet.html.'
    )
  )
  console.log()
  process.exit(1)
...
console.log()
console.log(
  pc.green(`${pc.bold('✓')} Project initialized for ${pc.bold(repoPath)}`)
)
console.log(pc.dim(`  (was: ${configuredPath})`))
console.log()
console.log(
  `  ${pc.cyan('package.json')}         name → ${pc.bold(repoName)}, repository URL updated`
)
