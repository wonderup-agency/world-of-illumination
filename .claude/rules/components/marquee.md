...
- **Extra spacing between two specific elements inside an item** (e.g. two headings sharing one `.marquee_item`, like the ticket banner's "PRESALE LIVE..." + "SAVE UP TO 30%" pair) is a different case from the item-to-item gap above — use `data-marquee-gap` directly on the second element instead of touching `marquee.css`. It's read once at init (see Behavior) and baked into `baseItems`, so it survives every clone/resize with no extra code. Works on any element inside `.marquee_item`, not just headings.
