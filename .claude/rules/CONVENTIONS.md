...
- **The same bundling risk applies to CSS imported from npm packages, not just Webflow class names.** If a component imports a library's own stylesheet (e.g. `testimonials.js` importing `swiper/css/navigation`), that library's bare selectors (`.swiper-button-lock`, `.swiper-button-disabled`, pagination bullet classes, etc.) also ship in the shared `dist/styles.css` and apply to *every* Swiper instance sitewide, not just the component that imported them. Other Swiper-based components using those same class names (even unintentionally, since Swiper adds them automatically) need scoped overrides — with `!important` if the library's own rule also uses it, or if it wins the cascade by load order — to avoid silently inheriting behavior they never asked for. See [`slider-swiper.md`](./components/slider-swiper.md#notes) for a case where this hid working nav arrows in an unrelated component.
